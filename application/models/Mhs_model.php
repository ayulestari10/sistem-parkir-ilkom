...
class Mhs_model extends CI_Model{
	...
	public function get_data($id_mhs)
	{
	...
	function get_data_angkatan($sort){
		$query = $this->db->query('SELECT * FROM '. $this->table .' WHERE angkatan ='.$sort );
		return $query->result();
	}

 	function get_data_jurusan($sort){
 		$query = $this->db->query('SELECT * FROM '. $this->table .' WHERE jurusan LIKE "%'.$sort.'%"' );
		return $query->result();
 	}

 	function get_data_az(){
 		$query = $this->db->query('SELECT * FROM '. $this->table .' ORDER BY nama ASC');
		return $query->result();
 	}

 	function get_data_za(){
 		$query = $this->db->query('SELECT * FROM '. $this->table .' ORDER BY nama DESC');
		return $query->result();
 	}
